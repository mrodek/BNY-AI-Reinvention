# Master Outline

## Metadata

- Working title: `<book title>`
- Source artifact: `02_outline/source_documents/<source file>`
- Original external path: `<optional external source path>`
- Extraction status: `<not_started | extracted | manually_reformatted>`
- Notes: `<short note on how this outline should be used>`

---

# <Book Title>

## Part I: <Part Name>

### Chapter 1: <Chapter Title>

- <key point>
- <key point>
- <key point>

Executive takeaway:
<one-sentence takeaway>

### Chapter 2: <Chapter Title>

- <key point>
- <key point>
- <key point>

Signal:
<one-sentence signal>

## Part II: <Part Name>

### Chapter 3: <Chapter Title>

- <key point>
- <key point>
- <key point>

Core idea:
<one-sentence core idea>

## Part III: <Part Name>

### Chapter 4: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 5: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 6: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 7: <Chapter Title>

- <key point>
- <key point>
- <key point>

Core belief:
<one-sentence core belief>

## Part IV: <Part Name>

### Chapter 8: <Chapter Title>

- <key point>
- <key point>
- <key point>

Truth:
<one-sentence truth>

### Chapter 9: <Chapter Title>

- <key point>
- <key point>
- <key point>

Executive outcome:
<one-sentence executive outcome>

### Chapter 10: <Chapter Title>

- <key point>
- <key point>
- <key point>

Positioning:
<one-sentence positioning>

## Part V: <Part Name>

### Chapter 11: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 12: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 13: <Chapter Title>

- <key point>
- <key point>
- <key point>

## Part VI: <Part Name>

### Chapter 13: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 14: <Chapter Title>

- <key point>
- <key point>
- <key point>

### Chapter 15: <Chapter Title>

- <key point>
- <key point>
- <key point>

Final message:
<one-sentence final message>

## Final Executive Insight

<summarize the full causal arc in 5 to 7 numbered points>
