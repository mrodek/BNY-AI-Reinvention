# Challenge Round: CH18 - The Runtime: Agents and Orchestration

## Purpose

Pressure-test CH18 `draft_v1` before moving to `draft_v2`. Focus on whether the chapter draws a crisp boundary from CH17, keeps the workflow-versus-agent distinction practical rather than fashionable, and sets up CH19 without collapsing into observability.
...
The draft ends well by saying the runtime generates behavior and observability interprets it. Still, some sentences about making workflows "intelligible while they run" edge close to implying that the runtime itself provides enough visibility for governance. An architect reading CH18 and CH19 together will want a cleaner statement that runtime state is necessary for execution control, while cross-run interpretation remains a separate institutional function.
...
### 5. Adjacent Chapter Reader (CH17 / CH19)

**The challenge:** The chapter sequence is strong, but the handoff logic could be made even more explicit.

CH17 governs each request, CH18 governs each multi-step execution path, and CH19 governs confidence over time. That sequence is already present, but it deserves one cleaner summary sentence near the end so the reader can feel the architecture progression before entering CH19.
...
Revise before moving to `draft_v2`. CH18's core argument is sound and already well aligned with CH17 and CH19. The revisions needed are targeted rather than structural:
...
- Notes: CH18 already works conceptually. `draft_v2` should be a tightening pass focused on design-choice clarity, risk-proportionate autonomy, and a sharper handoff into CH19.
